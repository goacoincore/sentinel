...
def is_valid_goacoin_address(address, network='mainnet'):
...
def SHIM_serialise_for_goacoind(sentinel_hex):
    ...
    # goacoind expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    goacoind_hex = serialise(obj)
    return goacoind_hex
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('goacoin.conf')
