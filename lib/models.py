...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, goacoind):
        golist = goacoind.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_goacoind(goacoind, item)
    ...
    @classmethod
    def import_gobject_from_goacoind(self, goacoind, rec):
        import decimal
        import goacoinlib
        import inflection
        ...
        # shim/goacoind conversion
        object_hex = goacoinlib.SHIM_deserialise_from_goacoind(object_hex)
        objects = goacoinlib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from goacoind...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from goacoind, with every run
        ...
        try:
            ...
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(goacoind)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from goacoind! %s" % e)
            govobj.vote_delete(goacoind)
            return (govobj, None)
    ...
    def vote_delete(self, goacoind):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(goacoind, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, goacoind, signal, outcome):
        import goacoinlib
        ...
        output = goacoind.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = goacoinlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(goacoind, signal)

    def sync_network_vote(self, goacoind, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = goacoind.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (goacoinlib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def get_prepare_command(self):
        import goacoinlib
        obj_data = goacoinlib.SHIM_serialise_for_goacoind(self.serialise())
    ...
    def prepare(self, goacoind):
        try:
            object_hash = goacoind.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not goacoinlib.is_valid_goacoin_address(addr, config.network):
    ...
    def hash(self):
        import goacoinlib
        return goacoinlib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    @classmethod
    def active(self, goacoind):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - goacoind.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, goacoind):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - goacoind.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, goacoind):
        now = int(time.time())
        return (self.created_at < (now - goacoind.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, goacoind):
        if self.is_expired(goacoind):
            return False

        return True

    def is_deletable(self, goacoind):
        if self.is_expired(goacoind):
            return True
