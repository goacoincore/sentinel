...
def test_valid_goacoin_address():
    from goacoinlib import is_valid_goacoin_address

    main = valid_goacoin_address()
    test = valid_goacoin_address('testnet')

    assert is_valid_goacoin_address(main) is True
    assert is_valid_goacoin_address(main, 'mainnet') is True
    assert is_valid_goacoin_address(main, 'testnet') is False

    assert is_valid_goacoin_address(test) is False
    assert is_valid_goacoin_address(test, 'mainnet') is False
    assert is_valid_goacoin_address(test, 'testnet') is True


def test_invalid_goacoin_address():
    from goacoinlib import is_valid_goacoin_address

    main = invalid_goacoin_address()
    test = invalid_goacoin_address('testnet')

    assert is_valid_goacoin_address(main) is False
    assert is_valid_goacoin_address(main, 'mainnet') is False
    assert is_valid_goacoin_address(main, 'testnet') is False

    assert is_valid_goacoin_address(test) is False
    assert is_valid_goacoin_address(test, 'mainnet') is False
    assert is_valid_goacoin_address(test, 'testnet') is False
...
def test_deterministic_masternode_elections(current_block_hash, mn_list):
    from goacoinlib import elect_mn
...
def test_parse_masternode_status_vin():
    from goacoinlib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % goacoinlib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(goacoinlib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(goacoinlib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(goacoinlib.blocks_to_seconds(16616)) == 2612035
