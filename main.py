...
from lib import goacoin_config
import time
import datetime
import sys
import os
...
def fix_masternode(data_folder):
    try:
        ...
        if not os.path.isfile(wallet_file):
            print('It seems like the data folder (the one containing wallet.dat) is not the same as the folder where goacoin.conf is')
            data_folder = input('Please, write the data folder path: ')
            return fix_masternode(data_folder)

        print(colored('Make a copy of "wallet.dat" and "goacoin.conf" before continuing.\nThis program will try it best not to touch them, but just in case!', attrs=['bold']))
        ...
        for filename in os.listdir(data_folder):
            if filename in ('wallet.dat', 'goacoin.conf'):
                continue
        ...
        print(colored('Checking goacoin.conf contents', 'green'))
        tokens = goacoin_config.GoacoinConfig.tokenize(config.goacoin_conf)
        ...
        try:
            ...
            r = None
            while r not in ('y', 'n'):
                print(colored('It seems like your goacoin.conf is not setting the wallet as masternode!', 'red'))
        ...
        if 'masternodeprivkey' not in tokens:
            print(colored('Your goacoin.conf does not contain a \'masternodeprivkey\' entry, please set it up before opening your wallet', 'red'))

        # Rewrite config
        with open(config.goacoin_conf, 'w') as fp:
            for k, v in tokens.items():
                fp.write('{}={}\n'.format(k, v))

        print(colored('Done checking\n', 'green'))
        print(colored('Please open your wallet (goacoind on linux) and wait for it completely load/open before continuing', attrs=['bold']))
        ...
        print(colored('If it says it can not connect, check your goacoin.conf settings', 'yellow'))
...
if __name__ == '__main__':
    ...
    print(colored('Using goacoin.conf: {}'.format(config.goacoin_conf), 'green'))
    ...
    if option == 1: 
        ...
        data_folder = os.path.dirname(config.goacoin_conf)
